  ...
  try{
    ...
    console.log("SignInUser - check1 response:", check1.data);
      ...
      if(check1.status === 200){
        const calculatedHash = await generateKey(data.password, check1.data.data.auth_salt);

        console.log("SignInUser - calculatedHash:", calculatedHash);

        const check2 = await axios.post(`http://${ip_address}:3002/api/signin/check2`,{
        ...
        if(check2.status === 200){
          ...
          console.log("SignInUser - Stored accessToken and userProfile in storage.");
          ...
          console.log("SignInUser - Generating passwordKey and storing user secret data.");

          console.log("SignInUser - userData:", userData.email,data.password, userData.secret.pk_salt);

          const passwordKey = await generateKey(userData.email + data.password, userData.secret.pk_salt);
          ...
          console.log("SignInUser - Storing masterKeyData and refreshToken in SecretStorage.");
          ...
          console.log("MasterKeyData stored in SecretStorage.",await SecretStorage.retrieveSecret('masterKeyData'));
          
          await SecretStorage.storeSecret('refreshToken', check2.data.tokens.refreshToken);
    ...
    if(axios.isAxiosError(err)){
      ...
      if(err.response?.data?.message === "No user found with this username or email"){
        return {
          type: 'error',
          field: 'userInput',
      ...
      else if(err.response?.data?.message === "Invalid credentials provided."){
        return {
          type: 'error',
          field: 'password',
