...
import { getExtensionFromMime } from "@/util/openWith";
...
export interface FilesMetaServiceTypes {
  ...
  getFilesDataByCategory(category: category): {
    totalFiles: number;
    totalSize: number;
  };
}
...
class FilesMetaService implements FilesMetaServiceTypes {
  ...
  getFilesDataByCategory(category: category) {
    const types = fileTypes(category);
    const filteredFiles = this.filesData.filesCache.filter((file) =>
      types.includes(getExtensionFromMime(file.fileType.toLowerCase())),
    );
