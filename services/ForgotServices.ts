...
class ForgotPasswordService {
  static async #getRecoverySalt(
    userInput: string,
  ): Promise<RecoverySaltResponse> {
    try {
      const res = await axios.post(
        `http://${ip_address}:3002/api/forgotpassword/validaterecovery/check1`,
        {
          userInput,
        },
      );
      ...
      if (res.status !== 200) {
        return {
          success: false,
          status: res.status,
          message: res.data.message || "Failed to get recovery salt",
        };
      }
      ...
      if (axios.isAxiosError(error) && error.response) {
        if (
          ...
          error.response.data.field === "userInput"
        ) {
          ...
          return {
            ...
            message:
              ...
              "Conflict error occurred while fetching recovery salt",
            status: 409,
          };
        }
        ...
        return {
          ...
          message:
            error.response.data.message ||
            "An error occurred while fetching recovery salt",
        };
      }
      console.error("Error fetching recovery salt:", error);
      return {
        success: false,
        message: "An error occurred while fetching recovery salt",
      };
    }
  }

  static async getResetToken(
    userInput: string,
    recoveryKeyPhrases: string,
    ...
    try {
      ...
      if (!check1.success && check1.status === 409) {
        ...
        return {
          success: false,
          message: check1.message || "Failed to get recovery salt",
        };
      }

      if (
        !check1.nonce ||
        !check1.recoveryKeyHashSalt ||
        !check1.userName ||
        !check1.rk_salt
      ) {
        return {
          success: false,
          message: "Failed to get required recovery data",
        };
      }

      console.log(
        `Using recovery hash salt: ${check1.recoveryKeyHashSalt} and nonce: ${check1.nonce}`,
      );
      ...
      const res = await axios.post(
        `http://${ip_address}:3002/api/forgotpassword/validaterecovery/check2`,
        {
      ...
      if (res.status !== 200) {
        return {
          success: false,
          message: res.data.message || "Failed to validate recovery key",
        };
      }
      ...
      if (axios.isAxiosError(error) && error.response) {
        if (
          error.response.status === 409 &&
          error.response.data.message === "Invalid recovery proof"
        ) {
          console.error("Conflict error response:", error.response.data);
          return {
            success: false,
            ...
            message:
              error.response.data.message ||
              "Conflict error occurred while fetching reset token",
            status: 409,
          };
        }
        ...
        return {
          ...
          message:
            error.response.data.message ||
            "An error occurred while fetching reset token",
        };
      }
  ...
  static async forgotPassword(
    ...
    try {
      const masterKey = await decryptData(
        data.recoveryData.encryptedRecoveryMasterKey,
        data.recoveryKey,
      );
      ...
      console.log(
        "Forgot - userData:",
        data.recoveryData.email,
        data.newPassword,
        new_pk_salt,
      );
      const newPasswordKey = await generateKey(
        data.recoveryData.email + data.newPassword,
        new_pk_salt,
      );

      const new_encryptedMasterKey = await encryptData(
        masterKey,
        newPasswordKey,
      );
      ...
      const res = await axios.post(
        `http://${ip_address}:3002/api/forgotpassword/resetpassword`,
        {
      ...
      return {
        success: false,
        message: "An error occurred while validating recovery key",
      };
    }
  }
}
