...
import { DeleteFile } from "@/util/filesOperations/fileDelete";
...
const FullItem = ({ item, animatedItem, onClose, category }: FullItemProps) => {
  const { userProfile, reload } = useContext(UserDataContext);
  ...
  const handleDeleteConfirm = async () => {
    setDeleting(true);
    if (!userProfile?.id || !item?._id) return;

    const res = await DeleteFile(userProfile.id, item._id);
    ...
    <Pressable onPress={onClose} style={StyleSheet.absoluteFill}>
      ...
      >
        ...
        <Pressable onPress={(e) => e.stopPropagation()}>
          {deleting ? (
            <View
              className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl overflow-hidden p-6 items-center justify-center"
              style={{ width: 300 }}
            >
              <ActivityIndicator
                size="large"
              ...
              />
              <Text className="text-lg font-roboto-medium text-gray-800 dark:text-gray-200 mb-4">
                Deleting...
              </Text>
            </View>
          ) : (
              ...
              <View className="border-t border-gray-200 dark:border-gray-700 flex-row relative">
                ...
                {showMenu && (
                  <View className="absolute bottom-16 right-2 bg-white dark:bg-gray-700 rounded-lg shadow-md z-50 min-w-max border border-gray-200 dark:border-gray-600 overflow-hidden">
                    ...
                    >
                      <Trash width={18} height={18} color="#dc2626" />
      ...
      <ConfirmDialog
        ...
        title="Delete File?"
        message={`Are you sure you want to delete "${item?.filename}"? This action cannot be undone.`}
        confirmText="Delete"
        cancelText="Cancel"
        variant="danger"
